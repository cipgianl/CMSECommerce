...
namespace CMSECommerce.Infrastructure.TagHelpers
{
        ...
        private string AddPageContent()
        {
            ...
            if (PageNumber <= PageRange)
            {
                for (int currentPage = 1; currentPage < 2 * PageRange + 1; currentPage++)
                {
                    ...
                    content.Append($"<li class='page-item {active}'><a class='page-link'href='{PageTarget}?categoryId={CategoryId}&p={currentPage}'>{currentPage}</a></li>");
                }
            }
            ...
            {
                content.Append($"<li class='page-item'><a class='page-link' href='{PageTarget}?categoryId={CategoryId}&p={PageCount}'>{PageLast}</a></li>");
            }
